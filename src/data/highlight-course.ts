export const highlight_course = {
  type: "Pós-Graduação",
  modality: "Presencial",
  title: "Cardio-oncologia",
  description: `A Pós-graduação Lato Sensu em Cardio-oncologia é a escolha ideal para médicos que buscam se especializar, destacar-se e adquirir as habilidades necessárias para atuar na área com segurança e excelência.`,
  coordenadorImageURL:
    "https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  coordenadorName: "Dr. João Silva",
  coordenadorDescription: "Advogado e Médico",
  price: "R$ 35.000,00",
  slug: "capacitacao-em-leitura-cientifica",
};
